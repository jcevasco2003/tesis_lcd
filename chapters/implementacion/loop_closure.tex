...
Implementamos el sistema de submapas propuesto en LoopSplat, realizando todas las operaciones que antes eran globales, como la optimización de las gaussianas en la imagen, en un marco local de cada submapa. 

De esta forma, cada submapa se construye a partir de un conjunto de fotogramas consecutivos, obteniendo así regiones reducidas. La idea es que a partir de cierto tiempo $t_i$, caracterizado por la integración de un keyframe nuevo lo suficientemente lejano al inicio del submapa actual, se cree un nuevo submapa. Así, se inicializará un nuevo conjunto de gaussianas propio del submapa nuevo, y se irán acumulando todas las mediciones visuales e inerciales siempre en el marco local del submapa. De esta forma, cada submapa tiene su propio conjunto de gaussianas, keyframes y su propia optimización, a cambio de cierto grado de redundancia en la representación. Esto último se debe a que dos submapas consecutivos inevitablemente compartirán información de la escena, así como también en otros submapas que lleguen a observar al menos parte de la misma región del espacio, en especial aquellos que forman ciclos.
...
Una vez obtenidas las correspondencias entre los submapas que cierran un ciclo, se actualiza el grafo de poses donde cada nodo representa la pose de un submapa y cada arista representa una restricción de transformación. La matriz de información de las aristas se calcula a partir de la incertidumbre acumulada por la odometría, la confianza devuelta por ICP, el coeficiente de superposición de las nubes y la similaridad devuelta por NetVLAD.

Para optimizar este grafo, se utiliza la biblioteca Ceres Solver \cite{Agarwal_Ceres_Solver_2022}, que permite resolver problemas de optimización no lineal en CPU. Debido a la reducción del número de nodos como submapas, la optimización no se ve afectada significativamente por el rendimiento de la CPU.