...
\noindent con $\mb{P}_k \in SE(3)$ representando la traslación $\mb{p}_k \in \mathbb{R}^3$ y la rotación $\mb{R}_k \in SO(3)$; $\mb{V}_k$ la velocidad; $\mb{b}_a^k$ y $\mb{b}_g^k$ los sesgos del acelerómetro y giróscopo, respectivamente.
...
\noindent donde $\mb{u}(u_x, u_y)$ es la ubicación del píxel en el plano bidimensional, y $\mb{u}_c$ su ubicación en el espacio tridimensional en el sistema de referencia de la cámara.
...
\noindent donde $\mathbf{p}_k$ es la posición, $\mathbf{q}_k$ la orientación, $\mathbf{v}_k$ la velocidad y $\mathbf{b}_{a_k},\mathbf{b}_{g_k}$ representan los sesgos del acelerómetro y el giroscopio respectivamente.
...
\noindent que representan los cambios relativos de posición, velocidad y orientación expresados en el sistema de referencia del instante $i$.
...
\noindent donde $\mathbf{R}_i$ es la matriz de rotación asociada al cuaternión $\mathbf{q}_i$.
...
\noindent donde cada componente corresponde a un error en posición, orientación, velocidad y sesgos.
...
\noindent donde $\mathrm{vec}(\cdot)$ extrae la parte vectorial del cuaternión.
...
\noindent es el Jacobiano del residuo respecto al estado.
...
\noindent mientras que respecto a $\mathbf{p}_j$
...
\noindent y respecto a la velocidad inicial
...
\noindent donde $\mathbf{W}=\boldsymbol{\Sigma}^{-1}$ es la inversa de la covarianza de la preintegración.
...
\noindent donde $\mathbf{r}(\mathbf{x})$ es el vector de residuos del sistema.
...
\noindent y $\mathbf{J}$ es el Jacobiano del residuo.
...
\noindent y el vector $\mathbf{b}$ como
...
\noindent que depende únicamente de las variables restantes $\mathbf{x}_r$.
...
\noindent donde $\mathbf{J}_{marg}$ es el jacobiano del nuevo factor de marginalización.
...
\noindent donde $\mathbf{x}_r^0$ es el punto de linearización almacenado en el momento de 
...
\noindent con $T_n$ la transmitancia, $\alpha_n$ la opacidad y $c_n$ el color de la gaussiana $n$ en orden de profundidad back-to-front.
...
\noindent con $C_{\leq n}$ el color acumulado hasta n.
...
\noindent y sea $\alpha = \sigma G$ la opacidad efectiva del splat en el píxel.
Entonces,
...
\noindent y el término correspondiente del gradiente queda
...
\noindent donde el indicador $\mathbf{1}_{\text{med}}(n)$ vale 1 únicamente en el píxel donde la gaussiana $n$ cruza $T=0.5$, y $\frac{\partial D_{\text{med}}}{\partial z_n} = 1$ ya que $D_{\text{med}}$ depende linealmente de $z_n$.
...
\noindent donde usamos que $d_n(p) = z_n + \hat{p}_n \cdot (p - \mu_n^{2D})$, de modo que $\frac{\partial d_n(p)}{\partial z_n} = 1$. 
...
\noindent donde $\mb{J}$ depende de la media 3D a través de la proyección perspectiva.
...
\noindent donde $\Delta_i = s_i - \bar{s}$.
