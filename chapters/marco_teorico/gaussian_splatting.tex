...
\noindent vale que la covarianza
...
\begin{align*}
max(G^1(t)) =& max(log(G^1(t))) \\ 
            =& max(-(o+tv-x_c)^T\Sigma^{-1}(o+tv-x_c)) \\
...
\noindent con $\theta$ el ángulo entre el rayo de luz y el eje principal de la cámara. Para simplificarlo, \cite{zhang2024radegs} aproxima por $\theta_c$, el ángulo definido por la media $x_c$ de la gaussiana. Así, la profundidad $x$ es
\begin{equation}
