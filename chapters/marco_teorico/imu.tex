...
\noindent con $\mathbf{w}_i$ ruidos gaussianos de media cero y la matriz $\Omega$ formada por el ratio angular estimado ${}_s\omega = {}_s\tilde{\omega} + \mathbf{w}_g - \mathbf{b}_g$ a partir de la medida ${}_s\tilde{\omega}$. En esta formulación, la actualización del cuaternión se interpreta como una perturbación a izquierda:
...
Al iniciar el robot, se lo deja inmóvil durante unos segundos. Luego, se calcula el promedio de las mediciones del acelerómetro y del giróscopo, utilizándolo como una estimación inicial del sesgo de ambos sensores. Si bien esta aproximación proporciona una inicialización adecuada, los sesgos varían con el tiempo debido a factores como la temperatura y el ruido, por lo que resulta necesario estimarlos continuamente mediante la fusión con otras fuentes de información. Históricamente, los filtros de Kalman \cite{MasnadiShirazi2019KalmanTutorial} y sus extensiones para sistemas no lineales, como el Filtro de Kalman Extendido (EKF), han sido ampliamente utilizados para este propósito. Sin embargo, el enfoque adoptado en este trabajo se basa en una formulación de optimización no lineal que estima conjuntamente la trayectoria, el mapa y los parámetros del sistema. En este contexto, la gran cantidad de gaussianas que representan el mapa introduce un elevado número de variables de estado, haciendo más natural integrar la estimación de los sesgos dentro del mismo problema de optimización. De este modo, los sesgos del acelerómetro y del giróscopo se corrigen conjuntamente con el resto de las variables mediante la optimización que fusiona las observaciones visuales e inerciales.