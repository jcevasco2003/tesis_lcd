...
Una variedad suave es un espacio topológico que, localmente, se asemeja a un espacio euclídeo de dimensión finita. Es suave en el sentido de diferenciable, por lo que, dicho de forma más sencilla, no tiene bordes ni picos. Este espacio suele encontrarse dentro de otro espacio bajo ciertas restricciones de estado. Tal es el caso de los vectores en $\mathbb{R}^3$ que cumplen con tener norma uno. Este espacio conforma una variedad esférica de radio uno y suave.

Un grupo $(\mathcal{G}, \circ)$ es un conjunto $\mathcal{G}$ con una operación binaria $\circ$, tal que satisface los siguientes axiomas:

Dados $X, Y, Z \in \mathcal{G}$ cualesquiera,
...
Existe un elemento $E \in \mathcal{G}$ tal que, para todo $X \in \mathcal{G}$,
\[
E \circ X = X \circ E = X.
\]

\item \textbf{Elemento inverso:} 
Para todo $X \in \mathcal{G}$ existe un elemento inverso $X^{-1} \in \mathcal{G}$ tal que
\[
X^{-1} \circ X
=
X \circ X^{-1}
...
Con los grupos de Lie, es posible transformar elementos de otros conjuntos actuando sobre ellos. En particular, como trabajamos con grupos de matrices, extendemos la acción de grupo a vectores y matrices mediante la multiplicación matricial.

Dado un Grupo de Lie $\mathcal{M}$ y un conjunto $\mathcal{V}$, se nota $X \cdot v$ a la acción de $X \in \mathcal{M}$ en $v \in \mathcal{V}$,
...
Existe un elemento identidad $E \in \mathcal{M}$ tal que, para todo $v \in \mathcal{V}$,
\[
E \cdot v = v.
\]

\item \textbf{Compatibilidad con la operación del grupo:}  
Para todo $X, Y \in \mathcal{M}$ y todo $v \in \mathcal{V}$,
\[
(X \circ Y) \cdot v
=
X \cdot (Y \cdot v).
...
Podemos pensar la operación $\circ$ como un caso particular de acción del grupo a sí mismo.
...
Denotamos el espacio tangente a un punto $X \in \mathcal{M}$ como $T_{X} \mathcal{M}$.
...
$$
X^{-1} \dot{X} + \dot{X}^{-1}X = 0.
$$

Así, los elementos del álgebra de Lie, es decir del espacio tangente a la identidad, son de la forma

\begin{equation}
v^{\wedge} = X^{-1} \dot{X} = - \dot{X}^{-1} X.
\label{derivada_algebra_lie}
\end{equation}

Los elementos del álgebra de Lie, notados $\tau^{\wedge}$, pueden ser expresados como combinación lineal de una base $E$ de $\mathfrak{m}$. Los elementos $E_i$ son llamados generadores y son las derivadas de $X$ alrededor del origen para cada dirección $i$. 
...
\noindent donde $\mathbf{e}_i$ son los vectores de la base de $\mathbb{R}^m$, con $\mathbf{e}_i^\wedge = E_i$.
...
El mapa exponencial surge al considerar las derivadas temporales de $X \in \mathcal{M}$ sobre la variedad. Despejando la ecuación \ref{derivada_algebra_lie}, tenemos que
...
$$
X(t) = X(0) \exp(v^\wedge t).
$$

Como $X(t)$ y $X(0)$ son elementos del grupo, entonces $\exp(v^\wedge t) = X(0)^{-1} X(t)$ lo es también. Luego, $\exp(v^\wedge t) $ mapea elementos $v^\wedge$ del álgebra de Lie al grupo.

Para los grupos multiplicativos, se puede obtener una forma cerrada del mapa exponencial mediante el desarrollo de Taylor:

\begin{equation}
\exp(\tau^\wedge) = E + \tau^\wedge + \frac{1}{2} {\tau^\wedge}^2 +
...
Del mismo modo que podemos hablar del mapa exponencial e inverso, podemos hablar de la versión en mayúscula Exp y Log, que comprimen mapear directamente vectores a puntos de la variedad, y viceversa. En la figura \ref{grafico_mapa_exponencial} se observa esquemáticamente la forma en que, con estos operadores, podemos viajar entre los distintos espacios.
...
Mediante la suma y resta podemos analizar incrementos en una variedad expresándolos en el espacio lineal a partir de los operadores Exp y Log. Como la operación del grupo no es conmutable, definimos a izquierda y a derecha según el orden de los operandos.

\begin{itemize}
\item 
$$
\oplus \text{ a derecha}: \qquad Y = X \oplus \\^X\tau \coloneq
                    X \circ \text{Exp}(^X\tau) \in \mathcal{M}.
$$

\item
$$
\ominus \text{ a derecha}: \qquad ^E\tau = Y \ominus X \coloneq
             \text{Log}(X^{-1} \circ Y) \in \mathcal{T}_X\mathcal{M}.
$$

\item 
$$
\oplus \text{ a izquierda}: \qquad Y = \\^X\tau \oplus X \coloneq
                    \text{Exp}(^E\tau) \circ X \in \mathcal{M}.
$$

\item 
$$
\ominus \text{ a izquierda}: \qquad ^E\tau = Y \ominus X \coloneq
             \text{Log}(Y \circ X^{-1}) \in \mathcal{T}_E\mathcal{M}.
$$
...
\begin{equation}
\begin{split}
\prescript{E}{}{\tau} \oplus X &= X \oplus \prescript{X}{}{\tau} \\\\
 %
\text{Exp}(\prescript{E}{}{\tau}^\wedge) X &= 
X \text{Exp}(\prescript{X}{}{\tau})
\\\\
%
\exp(\prescript{E}{}{\tau}X) =
X &\exp(\prescript{X}{}{\tau})^\wedge X^{-1} = 
\exp(X \prescript{X}{}{\tau}^\wedge X^{-1}) \\\\
 %
\prescript{E}{}{\tau}^\wedge &= 
X \prescript{X}{}{\tau}^\wedge X^{-1}
\end{split}
\end{equation}
...
\noindent llamado matriz adjunta. Se obtiene de aplicar el operador Vee a la adjunta
...
\noindent que permite representar rotaciones y productos cruz con su operación de grupo: la multiplicación matricial.
